...
impl BooksRepo for SqliteBooksRepo {
    ...
    fn get(&self, book_id: &str) -> Result<Option<LibraryItem>, String> {
        let conn = self.pool.get().map_err(|e| e.to_string())?;
        let mut stmt = conn
            .prepare("SELECT * FROM books WHERE book_id = ?1")
            .map_err(|e| e.to_string())?;
        let mut rows = stmt
            .query_map(params![book_id], row_to_library_item)
            .map_err(|e| e.to_string())?;
        match rows.next() {
    ...
    fn search(&self, query: &str) -> Result<Vec<LibraryItem>, String> {
        ...
        let mut stmt = conn
            .prepare("SELECT * FROM books WHERE title LIKE ?1 ESCAPE '\\' OR author LIKE ?1 ESCAPE '\\' ORDER BY last_opened_at DESC")
            .map_err(|e| e.to_string())?;
        let items = stmt
            .query_map(params![pattern], row_to_library_item)
            .map_err(|e| e.to_string())?
            .collect::<Result<Vec<_>, _>>()
            .map_err(|e| e.to_string())?;
        Ok(items)
    ...
    fn update_progress(
        ...
        let now = chrono::Utc::now().to_rfc3339();
        conn.execute(
            "UPDATE books SET progress_percent = ?1, last_opened_at = ?2, updated_at = ?3 WHERE book_id = ?4",
            params![percent, last_opened_at, now, book_id],
        )
        .map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn update_stats(&self, book_id: &str, stats: &ReadingStatsSnapshot) -> Result<(), String> {
        ...
        let now = chrono::Utc::now().to_rfc3339();
        conn.execute(
            "UPDATE books SET total_read_seconds = ?1, last_read_at = ?2, bookmark_count = ?3, last_chapter_index = ?4, updated_at = ?5 WHERE book_id = ?6",
            params![
                stats.total_read_seconds,
                stats.last_read_at,
                stats.bookmark_count,
                stats.last_chapter_index,
                now,
                book_id,
            ],
        )
